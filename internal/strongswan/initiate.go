...
func NewReinitiator(client *vici.ClientConn, logger zap.Logger) *Reinitiator {
	initiateWorkerChannel := make(chan initiateData)
	go initiateWorker(zap.L().Sugar().With("type", "initiateWorker"), client, initiateWorkerChannel)
	return &Reinitiator{
		client:                client,
		logger:                logger,
	...
	for _, childSA := range ikeSAStatus.ChildSA {
		...
		default:
			if i.shouldLog(initiate) {
				zap.L().Sugar().Infof("Skip initiating Child SA %s, because Child SA %s is being initiated", initiate.getFullName(), i.currentInitiate.getFullName())
...
func initiateWorker(logger zap.Logger, client *vici.ClientConn, workerChannel chan initiateData) {
	for {
		initiateData := <-workerChannel

		logger.Sugar().Infof("Initiating a Child SA for %s", initiateData.getFullName())
		err := client.Initiate(initiateData.ChildName, initiateData.IKEName, func(fields map[string]interface{}) {
			msg, _ := fields["msg"]
			logger.Sugar().With("strongswanFields", fields).Infof("Initiating log for %s: %s", initiateData.getFullName(), msg)
		})

		if err != nil {
			logger.Sugar().Errorf("got error trying to initiate Child SA %s: %s", initiateData.getFullName(), err)
			continue
		}
		logger.Sugar().Infof("Initiated new Child SA %s", initiateData.getFullName())
	}
}
